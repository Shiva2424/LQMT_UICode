...
export type LimitBreachProps = {
  classes?: any;
  totalamount: any;
  setTotalAmount: any;
}

const LimitBreachView: React.FC<LimitBreachProps> = props => {
    // const containerStyle = useMemo(() => ({ width: '100%', height: '100%' }), []);
    const containerStyle = useMemo(() => ({ width: '99%', height: '90%' }), []);
  const gridStyle = useMemo(() => ({ height: '88%', width: '100%', margin: '10px' }), []);
  const [rowData, setRowData] = useState([]);
  useEffect(() => {
    fetch("http://localhost:8080/api/liquidity/limit-breached").then(res => res.json()).then(res => setRowData(res));
  },[]);
  const columnDefs: any = [
    {field: 'counterParty', editable: false.valueOf, headerClass: "ag-center-header", cellClass: "ag-center-cell"},
...
export default LimitBreachView;
